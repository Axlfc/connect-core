...
1.  **Para A-01 (Fortalecer Hashing - CRÍTICO):**
    *   **Solution:** Incrementar el número de iteraciones en `authelia/configuration.yml` a un valor seguro. El valor recomendado por Authelia es `2`, pero `3` o `4` ofrecen un balance aún mejor entre seguridad y rendimiento.
        ```diff
...
2.  **Para A-02 (Asegurar Cookie de Sesión - CRÍTICO):**
    *   **Solution:** Cambiar el valor de `secure` a `true` en la configuración de la cookie. Esto debe hacerse antes de cualquier despliegue en un entorno que utilice HTTPS.
        ```diff
        --- a/authelia/configuration.yml
        +++ b/authelia/configuration.yml
        @@ -40,7 +40,7 @@
...
3.  **Para A-03 (Reducir Duración de Sesión):**
    *   **Solution:** Reducir el valor de `remember_me_duration` a un periodo más conservador, como `1M` (un mes) o `14d` (dos semanas), para limitar la ventana de exposición en caso de compromiso de la sesión.
        ```diff
        --- a/authelia/configuration.yml
        +++ b/authelia/configuration.yml
        @@ -32,7 +32,7 @@
