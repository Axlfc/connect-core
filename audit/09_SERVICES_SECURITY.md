...
#### 🔧 Solución Sugerida (n8n)

*   **Solución Inmediata:** Modificar `n8n-task-runners.json` para implementar una política de "deny-list" o "allow-list" estricta. Denegar explícitamente el acceso a módulos peligrosos del sistema.
    ```json
...
#### 🔧 Solución Sugerida (Ollama)

*   **Solución:** Mejorar el `ollama-proxy` para que actúe como un verdadero "API Gateway".
...
#### 🔧 Solución Sugerida (ComfyUI)

*   **Solución:** Habilitar la autenticación nativa de ComfyUI como segunda capa de defensa.
...
#### 🔧 Solución Sugerida (Matrix)

*   **Para S-matrix-01:** Ver la solución detallada en `04_DOCKER_SECURITY.md`.
*   **Para S-matrix-02:**
    *   **Solución:** Cambiar el valor por defecto en `.env.example` y `.env.staging` a `false`. El registro de usuarios debe ser una acción explícita y controlada por el administrador.
        ```diff
...
#### 🔧 Solución Sugerida (Qdrant)

*   **Solución:** Habilitar la autenticación por API key.
