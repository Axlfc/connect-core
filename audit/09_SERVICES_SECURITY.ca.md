...
#### 🔧 Solució Sugerida (n8n)

*   **Solució Inmediata:** Modificar `n8n-task-runners.json` para implementar una política de "deny-list" o "allow-list" estricta. Denegar explícitamente el acceso a módulos peligrosos del sistema.
    ```json
...
#### 🔧 Solució Sugerida (Ollama)

*   **Solució:** Mejorar el `ollama-proxy` para que actúe como un verdadero "API Gateway".
...
#### 🔧 Solució Sugerida (ComfyUI)

*   **Solució:** Habilitar la autenticación nativa de ComfyUI como segunda capa de defensa.
...
#### 🔧 Solució Sugerida (Matrix)

*   **Para S-matrix-01:** Ver la solución detallada en `04_DOCKER_SECURITY.md`.
*   **Para S-matrix-02:**
    *   **Solució:** Cambiar el valor por defecto en `.env.example` y `.env.staging` a `false`. El registro de usuarios debe ser una acción explícita y controlada por el administrador.
        ```diff
...
#### 🔧 Solució Sugerida (Qdrant)

*   **Solució:** Habilitar la autenticación por API key.
