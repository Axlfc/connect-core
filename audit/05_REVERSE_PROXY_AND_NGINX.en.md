...
1.  **Para RP-01 y RP-02 (Añadir Headers y Hardening de SSL/TLS):**
    *   **Solution:** Crear un nuevo archivo de configuración global, por ejemplo `nginx-proxy/conf.d/hardening.conf`, y añadir las siguientes directivas. Esto aplicará una política de seguridad sólida a todos los servicios detrás del proxy.
        ```nginx
...
2.  **Para RP-03 (Implementar Rate Limiting):**
    *   **Solution:** Añadir una configuración de `limit_req_zone` en el mismo archivo `hardening.conf` para establecer límites globales.
        ```nginx
