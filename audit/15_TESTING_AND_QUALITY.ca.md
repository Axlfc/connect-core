...
1.  **Para T-01 y T-03 (Implementar una Estrategia de Testing Real):**
    *   **Solució:** Adoptar un framework de testing estándar como `pytest` para el código Python.
...
2.  **Para T-02 (Hacer Cumplir la Calidad en CI):**
    *   **Solució:** Eliminar `continue-on-error: true` de todos los pasos de linting en `.github/workflows/validate.yml`.
        ```diff
...
3.  **Integrar Tests en el Pipeline de CI:**
    *   **Solució:** Añadir un nuevo "job" al `validate.yml` o crear un nuevo workflow (`test.yml`) que ejecute las pruebas de `pytest`.
        ```yaml
