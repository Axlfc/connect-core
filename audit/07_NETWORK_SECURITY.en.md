...
1.  **Aislamiento de Services por Red:**
...
1.  **Para N-01 y N-02 (Rediseñar la Estrategia de `fail2ban`):**
    *   **Solution Ideal (Recomendada):** Eliminar el contenedor de `fail2ban`. Instalar y configurar `fail2ban` **directamente en el sistema operativo del host**. Esto le da acceso legítimo y seguro a los logs y a las `iptables` del host sin romper el modelo de seguridad de Docker.
    *   **Solution Alternativa (Si debe ser en contenedor):**
...
3.  **Para N-04 (Control de Egress):**
    *   **Solution:** Crear una red de Docker dedicada para el acceso a internet y adjuntar solo a los contenedores que lo necesiten explícitamente.
