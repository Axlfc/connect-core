...
1.  **Para AS-01 (Validación de Entradas):**
    *   **Solució:** Añadir una validación simple en `init_env.sh` para asegurar que los valores introducidos no contengan caracteres peligrosos. Se pueden envolver las variables en comillas para mayor seguridad.
        ```bash
...
2.  **Para AS-02 (Modificación Segura del `.env`):**
    *   **Solució:** Utilizar un enfoque más seguro que no dependa de `sed` para parsear el archivo. Una alternativa es leer el archivo línea por línea, modificar la línea deseada en memoria y luego reescribir el archivo completo.
        ```bash
