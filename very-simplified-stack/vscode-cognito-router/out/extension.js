...
async function callPreviewRoute(userTask, editor) {
...
function showRoutingPanel(userTask, preview) {
    const panel = vscode.window.createWebviewPanel('cognitoRouter', 'Cognito Route Decision', vscode.ViewColumn.Two, { enableScripts: true });
...
function getWebviewContent(userTask, preview) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="card">
        ...
        <ul>
            ${preview.reasons.map((r) => `<li>${r}</li>`).join('')}
        </ul>
    </div>
